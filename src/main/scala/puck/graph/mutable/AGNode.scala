...
class AGNode[Kind <: NodeKind[Kind]] (val graph: AccessGraph[Kind],
  ...
  object content {
    ...
    private val content0 : smutable.Set[NodeType] = smutable.Set()

    def apply() : smutable.Iterable[NodeType] = content0

    implicit def iterableContent(c : AGNode.this.content.type ) : smutable.Iterable[NodeType] = content0
  ...
  private [this] val superTypes0 : smutable.Set[NodeType] = smutable.Set()
  ...
  private val subTypes0 : smutable.Set[NodeType] = smutable.Set()
  ...
  object usesObject {
    private [AGNode] val uses0 = smutable.Set[NodeType]()
    def apply() : smutable.Iterable[NodeType] = uses0
    implicit def iterableUses(c : AGNode.this.uses.type ) : smutable.Iterable[NodeType] = uses0
  ...
  object usersObject {
    private val users0 = smutable.Set[NodeType]()
    def apply() : smutable.Iterable[NodeType] = users0

    implicit def iterableUsers(c : AGNode.this.users.type ) : smutable.Iterable[NodeType] = users0
  ...
  /*private[this] lazy val abstractions0: smutable.Set[(NodeType, AbstractionPolicy)] =
    searchExistingAbstractions()
  def searchExistingAbstractions() = smutable.Set[(NodeType, AbstractionPolicy)]()*/

  private[this] val abstractions0 = smutable.Set[(NodeType, AbstractionPolicy)]()

  def abstractions : smutable.Iterable[(NodeType, AbstractionPolicy)] = abstractions0
