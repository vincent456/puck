...
( source : NodeId,
  ...
  def withAccessKind(accessKind: Option[UsesAccessKind]) = Uses(source, target, accessKind)
...
( source : NodeId,
  ...
  def withAccessKind(accessKind: Option[UsesAccessKind]) = ParameterizedUses(source, target, accessKind)
}
