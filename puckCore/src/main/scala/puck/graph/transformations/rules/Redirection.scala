...
object Redirection {
  ...
  def redirectUsesAndPropagate(g  : DependencyGraph,
    ...
    (newUsed, oldUse.accessKind)  match {
      case (AccessAbstraction(absId, pol), _) =>
        redirectUsesAndPropagate(g, oldUse, absId, pol, keepOldUse)
      case (ReadWriteAbstraction(Some(rid), _), Some(Read)) =>
        redirectUsesAndPropagate(g, oldUse, rid, DelegationAbstraction, keepOldUse)
      case (ReadWriteAbstraction(_, Some(wid)), Some(Write)) =>
        redirectUsesAndPropagate(g, oldUse, wid, DelegationAbstraction, keepOldUse)
      case (ReadWriteAbstraction(Some(rid), Some(wid)), Some(RW)) =>
        redirectUsesAndPropagate(g, oldUse, rid, DelegationAbstraction, keepOldUse = true).flatMap {
          redirectUsesAndPropagate(_, oldUse, wid, DelegationAbstraction, keepOldUse)
        }
  ...
  def redirectUsesAndPropagate(g  : DependencyGraph,
    ...
    if(oldUse.used == newUsed) lg1.toLoggedEither
      ...
      oldUsedKind match {
        case TypeConstructor if g.kindType(newUsed) != TypeConstructor =>
