...
%:-use_module(sigmaRules, 
	     [abstract_type/2,
	      can_contain/2 as can_contain_rule,
	      abstract_uses/5]).
...
%%% if intro classe -> put isa and not uses and change the direction of the use
...
can_contain(node(_,HostType,_,_), node(_,Type,_,_)):- can_contain_rule(HostType,Type).
