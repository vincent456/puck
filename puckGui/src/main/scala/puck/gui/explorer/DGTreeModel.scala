...
object DGTreeModel{
  ...
  def focused(graph:DependencyGraph, edge: DGEdge) : DGTreeModel = {
    val g = DependencyGraph.subGraph(graph, Set(edge.source, edge.target))
    new FullDGTreeModel(g.addEdge(edge))
  }
  def subGraph(fullGraph : DependencyGraph, focus : Set[NodeId]): DGTreeModel ={
    new FullDGTreeModel(DependencyGraph.subGraph(fullGraph,focus))
  }
}
